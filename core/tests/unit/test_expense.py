...
def test_negative_amount_raises_error():
    ...
    with pytest.raises(InvalidAmountError):
        Expense(id=1, title="expense1", amount=-1, description="", expense_date=date.today())
...
def test_future_date_raises_error():
    ...
    """
    with pytest.raises(InvalidAmountError):
        Expense(id=1, title="expense1", amount=-1, description="", expense_date=(date.today() + timedelta(days=1)))
